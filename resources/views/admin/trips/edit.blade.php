        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900">
                <div class="flex justify-between items-center mb-6">
                    <h2 class="text-2xl font-bold">Edit Trip: {{ $trip->name }}</h2>
                ...
                <form method="POST" action="{{ route('admin.trips.update', $trip) }}">
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-4">
                        <div>
                            ...
                            <select name="origin_station_id" id="origin_station_id" 
                                ...
                                @foreach($stations as $station)
                                    <option value="{{ $station->id }}" {{ old('origin_station_id', $trip->origin_station_id) == $station->id ? 'selected' : '' }}>
                                        {{ $station->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="destination_station_id" id="destination_station_id" 
                                ...
                                @foreach($stations as $station)
                                    <option value="{{ $station->id }}" {{ old('destination_station_id', $trip->destination_station_id) == $station->id ? 'selected' : '' }}>
                                        {{ $station->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="mb-6">
                        ...
                        <div id="stops-container">
                            @php
                                $existingStops = old('stops', $trip->tripStops->pluck('station_id')->toArray());
                            @endphp
                            @if($existingStops)
                                @foreach($existingStops as $index => $stopId)
